...
class HXMusicEngine {
    ...
    synchronized boolean initMusicEngine(HXMusicItem music, final int position,
                                         final boolean isGapless, final boolean isLooped, final Context context) {
    ...
    int getCurrentProgress() {
        try {
            int currentProgress = currentPlayer.getCurrentPosition();
            return currentProgress;
        } catch (Exception e) {
            HXLog.e(LOG_TAG, "ERROR: not isPlaying(): " + e.getLocalizedMessage());
            return 0;
        }
    }
